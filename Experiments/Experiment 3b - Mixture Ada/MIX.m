...
cd('C:\Dropbox\Projects\Closure\Experiments\Experiment 3b - Mixture Ada')
...
for j=1:m
    ...
    for k=1:l
        ...
        %n_sig_h = zeros(n_mc,1);
        for i=1:n_mc
            X = mu_v+randn(n,1);
            P = 1-normcdf(X);
            
            n_sig_f(i) = fct(P,'mix-simes-hc',s/2,n);
            n_sig_s(i) = fct(P,'mix-simes-hc',s,n);
            n_sig_w(i) = fct(P,'mix-simes-hc',min(2*s,n),n);
        end
        toc(ti);
    ...
    legend([h1,h2,h3],{'s/2','s','2s'},'location','Best')
    
    if savefigs==1
        filename = sprintf( './FCT-sparse-n=%d-effect-size=%.2f.png',n,effect_size);
